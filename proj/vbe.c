...
int(init_graphic_mode)(uint16_t mode) {
  ...
  unsigned int vram_size = get_x_res() * get_y_res() * calculate_size_in_bytes(get_bits_per_pixel()); // VRAM's size, but you can use the frame-buffer size, instead
  ...
  if ((video_mem == MAP_FAILED) || (video_mem2 == MAP_FAILED)) {
    panic("Couldn't map video memory\n");
    return 1;
  }

  mapped_mem = video_mem;   //store the mapped memory pointer in mapped_mem (convert from void* to uint8_t*)
  mapped_mem2 = video_mem2; // store the video_mem2 in mapped_mem2
...
int(vg_draw_rectangle_on)(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint32_t color, uint8_t* buffer) {
  ...
  while ((currY < y + height) && (currY < get_y_res())) { // check if currY surpasses the limit, // need to use < instead of <= since it starts counting on 0
    if (vg_draw_hline_on(x, currY, width, color, buffer) != 0) {
      printf("Error on vg_draw_hline\n");
    }
    currY++;
  }
...
int(vg_draw_hline_on)(uint16_t x, uint16_t y, uint16_t len, uint32_t color, uint8_t* buffer) {
  ...
  if (get_memory_model() == DIRECT_COLOR_MODE) { // Direct Color
    ...
    //printf("3");
    memset(buffer + y * get_x_res() + x, (uint8_t) color, ((len > (get_x_res() - x)) ? get_x_res() - x : len)); // paint until the end of the line
  }
  else {
    printf("Invalid color mode\n");
    return 1;
...
int(draw_image)(uint16_t x, uint16_t y, xpm_image_t *img, enum xpm_image_type type) {
  ...
  for (uint16_t yIdx = y; yIdx < y + height; yIdx++) {
    ...
    for (uint16_t xIdx = x; xIdx < x + width; xIdx++) {
      ...
      uint32_t color;
      memcpy(&color, &bytes[byte_color_counter], pixel_size);
...
void swap_buffers(uint8_t *buffer) {
  memcpy(mapped_mem, buffer, get_x_res() * get_y_res() * calculate_size_in_bytes(get_bits_per_pixel()));
}

void clear_buffer(uint8_t *buffer, uint8_t color) {
  memset(buffer, color, get_x_res() * get_y_res() * calculate_size_in_bytes(get_bits_per_pixel())); // copies color to all bytes from buffer
}
...
int(set_display_start)(uint16_t buffer_y_start) {
  ...
  reg86.bh = 0x00;
  reg86.bl = /*0;*/ SET_DISPLAY_START_VERTICAL_RETRACE;
  reg86.cx = 0x00;           // x pos
  reg86.dx = buffer_y_start; // y pos

  if (sys_int86(&reg86) != OK) {
    printf("Error on set_displat_start -> sys_int86()\n");
    return 1;
  }
